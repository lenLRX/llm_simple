...
Llama2EmbeddingLayerNPUImpl::Forward(std::shared_ptr<Tensor> input,
                                     Llama2InferenceCtx &ctx) {
...
std::shared_ptr<Tensor>
RMSNormLayerNPUImpl::Forward(std::shared_ptr<Tensor> input,
                             Llama2InferenceCtx &ctx) {
  ...
  npu_rmsnorm_layer((void *)output_ptr, (void *)norm_weight, (void *)input_ptr,
                    ctx.cur_size, hidden_dim, eps, DT_FLOAT16, ctx.npu_stream);
  if (ctx.model->debug_print) {
    CHECK_ACL(aclrtSynchronizeStream(ctx.npu_stream));
  }
...
bool RMSNormLayerNPUImpl::Init(Llama2Model *model, int layer_no, bool pre_norm,
                               bool last_norm) {
  hidden_dim = model->hidden_dim;
  eps = model->norm_eps;
...
std::shared_ptr<Tensor>
SoftmaxLayerNPUImpl::Forward(std::shared_ptr<Tensor> input,
                             Llama2InferenceCtx &ctx) {
...
bool RoPELayerNPUImpl::Init(Llama2Model *model,
...
Llamma2TransformerLayerNPUImpl::Forward(std::shared_ptr<Tensor> input,
  ...
  if (true) {
    npu_flash_attn_opt_prefill_layer(tmp_output_tensor->data_ptr, q_emb->data_ptr,
                         k_cache->data_ptr, v_cache->data_ptr, ctx.cur_size,
                         ctx.cur_pos, ctx.prev_pos, n_heads, head_dim,
                         DT_FLOAT16, ctx.npu_stream);
  } else {
...
std::shared_ptr<Tensor>
MatmulLayerNPUImpl::Forward(std::shared_ptr<Tensor> input,
                            Llama2InferenceCtx &ctx) {
  ...
  spdlog::debug("MatmulLayerNPUImpl::Forward m {} n {} k {}", ctx.cur_size, n,
                k);
  ...
  if (qtype == QuantType::NoQuant) {
    ...
    npu_matmul_nz_layer((void *)output_ptr, (void *)input_ptr, (void *)weight,
                        ctx.cur_size, n, k, DT_FLOAT16, ctx.npu_stream);
...
bool MatmulLayerNPUImpl::Init(Llama2Model *model,
                              const std::string &weight_path, size_t n,
                              size_t k) {
...
bool MatmulLayerNPUImpl::InitAWQ(Llama2Model *model,
                                 const std::string &weight_path,
